...
test('date-range calendar opens on selected month and year', async ({ page }) => {
  ...
  const dateRangeField = frame.locator('[data-container-id="dateFieldIdThree"]');
  const dateRangeInput = dateRangeField.locator('.text-input-native-input');
  const calendarIcon = dateRangeField.locator('svg').first();
...
test('date-range calendar respects controlled viewDate', async ({ page }) => {
  await page.goto('/?path=/story/admiral-2-1-form-field-datefield--date-field-with-button-today&args=type:date-range');
  const frame = getStorybookFrameLocator(page);
  const dateRangeField = frame.locator('[data-container-id="dateFieldWithButtonToday"]');
  const dateRangeInput = dateRangeField.locator('.text-input-native-input');
  const calendarIcon = dateRangeField.locator('svg').first();
  const panel = frame.locator('.ui-kit-calendar-panel-component');
