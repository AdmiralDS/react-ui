...
import { Tree, TreeNode, T, UnorderedList, ListItem } from '@admiral-ds/react-ui';
...
const treeModel: Array<TreeItemProps> = [
  {
  ...
  },
  {
    ...
    children: [
      {
        render: (options: TreeNodeRenderOptionProps) => (
          <TreeNode {...options} label={'Текст  строки с чекбоксом, пятый уровень компонента 1'} key={'4-1'} />
        ),
        id: '5-1',
        checked: false,
      },
      {
        render: (options: TreeNodeRenderOptionProps) => (
          <TreeNode {...options} label={'Текст  строки с чекбоксом, пятый уровень компонента 2'} key={'4-2'} />
        ),
        id: '5-2',
        checked: false,
      },
...
export const TreeWithCheckStrictlyTemplate = (props: TreeProps) => {
  const [dataList, setDataList] = useState<TreeItemProps[]>(treeModel);
  ...
  return (
    <>
      <T font="Body/Body 1 Long" as="div" style={{ marginBottom: '24px' }}>
        Параметр checkStrictly отключает связь между родительскими и дочерними элементами.
        <br />
        При использовании этого режима:
        <UnorderedList>
          <ListItem>Родительский элемент становится обычным элементом с чекбоксом, независимым от дочерних.</ListItem>
          <ListItem>Выбор родительского элемента не влечёт выбор дочерних.</ListItem>
          <ListItem>
            Выбор дочернего элемента не переводит родительский в состояние indeterminate (неопределённое состояние).
          </ListItem>
        </UnorderedList>
      </T>
      <Tree
