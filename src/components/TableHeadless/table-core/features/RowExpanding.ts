...
export const RowExpanding: TableFeature = {
  ...
  getDefaultOptions: <TData extends RowData>(table: Table<TData>): ExpandedOptions<TData> => {
    return {
  ...
  createRow: <TData extends RowData>(row: Row<TData>, table: Table<TData>): void => {
    ...
    row.getIsExpanded = () => {
      ...
      return !!(table.options.getIsRowExpanded?.(row) ?? (expanded === true || expanded?.[row.id]));
    };
    row.getCanExpand = () => {
      return table.options.getRowCanExpand?.(row) ?? ((table.options.enableExpanding ?? true) && !!row.subRows?.length);
    };
