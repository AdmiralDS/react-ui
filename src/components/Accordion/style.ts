...
//   }
// `;
export const Chevron = styled(ChevronDownOutline).attrs(applyDefaultTheme)`
  transition: all 0.3s;
  flex-shrink: 0;
  width: 24px;
  height: 24px;
...
//   }
// `;
export const ItemTitle = styled.button.attrs(applyDefaultTheme)`
  position: relative;
  display: flex;
  justify-content: space-between;
  align-items: center;
...
//   }
// `;
export const ItemWrapper = styled.div.attrs<{ $opened?: boolean }>(applyDefaultTheme)`
  border-bottom: 1px solid var(--admiral-color-Neutral_Neutral20, ${(p) => p.theme.color['Neutral/Neutral 20']});
  & > ${ItemTitle} ${Chevron} {
    transform: ${(p) => (p.$opened ? 'rotate(180deg)' : 'rotate(0deg)')};
  }
...
//   }
// `;
export const ItemContent = styled.div.attrs<{ $contentMaxHeight: number | string }>(applyDefaultTheme)`
  color: var(--admiral-color-Neutral_Neutral90, ${(p) => p.theme.color[DefaultFontColorName]});
  overflow-y: auto;
  max-height: ${(p) => p.$contentMaxHeight};
  padding: 4px 16px 16px 16px;
