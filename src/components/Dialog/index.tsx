...
export const Dialog = forwardRef<HTMLDialogElement, ModalProps>(
  (
    {
    ...
    /** MutationObserver and open attribute */
    useEffect(() => {
    ...
    /** container - больше не нужен, так как диалог рисуется в top layer */
    // useEffect(() => {
    //   if (dialogRef.current) {
    ...
    const handleKeyDown = (event: React.KeyboardEvent<HTMLDialogElement>) => {
      ...
      if (event.key === 'Tab') {
        // focus trap
        const focusableEls: any = getKeyboardFocusableElements(dialogRef.current);
        if (event.shiftKey) {
          /* shift + tab */
          if (document.activeElement === focusableEls[0] || document.activeElement === dialogRef.current) {
    ...
    useEffect(() => {
      ...
      if (dialog) {
        dialog.addEventListener('close', onCloseCallback);
        return () => {
          dialog.removeEventListener('close', onCloseCallback);
        };
      }
    }, [onClose]);
